export const animationValues: string[] = [
  'fade',
  'slide',
  'flip',
  'scale',
...
export const colorModes: string[] = ['light', 'dark'];
...
export function getRandomImageNumber(type: string, sections: string) {
  const categoryLimits = limits[sections as keyof typeof limits] || limits.default;
  if (!categoryLimits || !categoryLimits[type as keyof typeof categoryLimits]) {
    throw new Error(`Limites non définies pour la catégorie "${sections}" et le type "${type}"`);
  }

  const [min, max] = categoryLimits[type as keyof typeof categoryLimits];
...
export function addRandomIndex(items: string[]) {
  return items.map(item => `${item}_${Math.floor(Math.random() * 2) + 1}`);
}

export function getRandomValue<T>(...args: T[]): T {
  return args[Math.floor(Math.random() * args.length)];
}

export function shuffleValue(array: string[]) {
