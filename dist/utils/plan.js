import { contentPosition, getRandomCategory, getRandomImageNumber, getRandomUniqueOrder, typeAssets, } from './index';
// PLAN
function getRandomCodePlan() {
    const codes = ['p1', 'p2'];
    return codes[Math.floor(Math.random() * codes.length)];
}
export function generateXsItems(total) {
...
export function addRandomImagesToItems(themeName, items, sections) {
    ...
    for (let i = 0; i < numberOfImages; i += 1) {
        ...
        items[randomIndex].image = `${category}/${typeAssets}/${getRandomImageNumber(typeAssets, category || '')}.png`;
    }
    return items;
}
export function getRandomPlan(themeName, sections) {
