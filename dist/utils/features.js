import { getRandomCategory, shuffleValue, addRandomIndex, getRandomUniqueOrder, getRandomBoolean, getRandomImageNumber, buttonVariant, buttonStyle, typeAssets, itemsFeatures, itemsFeaturesConverter, } from './index';
// FEATURE
function getRandomCodeFeature() {
    const codes = ['f1'];
    return codes[Math.floor(Math.random() * codes.length)];
}
export function getRandomFeature(themeName, sections) {
    const category = themeName === 'multi-portal' ? getRandomCategory() : sections;
    const shuffledFeatures = themeName === 'converter-portal'
